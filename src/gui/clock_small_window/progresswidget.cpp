...
lon::clock_window::ProgressWidget::ProgressWidget(QWidget* parent) : DisplayClockBase(parent) {
    qWarning() << "it is not wished that do not set widget width and height";
    // throw std::logic_error("wish to set height and width");
}

lon::clock_window::ProgressWidget::ProgressWidget(uint16_t height, uint16_t width, QWidget* parent)
    ...
    current_progress_ =
        scene_->addRect(-1 * width_, 0, width_, height_, *pen_, *working_foreground_brush_);
...
void lon::clock_window::ProgressWidget::updateTimeDisplay(TimerStatus const* timer_status) {
    ClockTime const* total_time = timer_status->getTotalTime();
    int current_progress_width = static_cast<int>(currentPersent(timer_status) * width_);
    ...
    if (total_time_ != total_time) {
        ...
        if (total_time == timer_status->clock_options()->work_time()) {
            ...
            current_progress_ = scene_->addRect(
                -1 * width_, 0, width_, height_, *pen_, *shortbreaking_foreground_brush_);
        } else {
            current_progress_ = scene_->addRect(
                -1 * width_, 0, width_, height_, *pen_, *longbreaking_foreground_brush_);
        }
