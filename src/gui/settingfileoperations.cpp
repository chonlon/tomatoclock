...
lon::ClockOptions SettingFileOperations::readClockOptionFromFile() {
    ...
    QJsonDocument json_document = QJsonDocument::fromJson(in_string.toLatin1(), &error);
    if (error.error == QJsonParseError::NoError) {
        if (!(json_document.isNull() || json_document.isEmpty()) && json_document.isObject()) {
            ...
            if (!data[QLatin1String("errorCode")].toInt()) {
                ...
                if (clock_settings.find(QLatin1String("WorkTime")) != clock_settings.end())
                    work = clock_settings[QLatin1String("WorkTime")].toInt();
                if (clock_settings.find(QLatin1String("ShortbreakTime")) != clock_settings.end())
                    shortbreak = clock_settings[QLatin1String("ShortbreakTime")].toInt();
                if (clock_settings.find(QLatin1String("LongbreakTime")) != clock_settings.end())
                    longbreak = clock_settings[QLatin1String("LongbreakTime")].toInt();
                if (clock_settings.find(QLatin1String("TimesBetweenLong")) != clock_settings.end())
                    shortbreak_times = clock_settings[QLatin1String("TimesBetweenLong")].toInt();
                if (clock_settings.find(QLatin1String("KeepWorking")) != clock_settings.end())
