...
lon::AddLabelWidget::AddLabelWidget(const std::list<QString> labels,
    ...
    line_edit_p_->setPlaceholderText(
        QString::fromLocal8Bit("请输入标签名, 标签名不可重复."));
...
void lon::AddLabelWidget::showMessage(const QString &message) {
    ...
    connect(messagebox_p_, SIGNAL(okButtonClicked()), this,
            SLOT(closeMessagebox()));
    connect(messagebox_p_, SIGNAL(cancelButtonClicked()), this,
            SLOT(closeMessagebox()));
    connect(messagebox_p_, SIGNAL(closeButtonClicked()), this,
