...
class SettingWidgetPrivate : public QWidget {
  ...
  public:
    explicit SettingWidgetPrivate(QWidget *parent = nullptr)
        ...
        connect(ui->work_slider_p_, SIGNAL(valueChanged(int)), this,
                SLOT(setWorkTimeLabel(int)));
        connect(ui->shortbreak_slider_p_, SIGNAL(valueChanged(int)), this,
  ...
  private:
    void setSliderQss(QSlider *slider, QString normal_color,
                      QString groove_color, QString handle_color) {
        int         sliderHeight = 8;
        int         sliderRadius = 4;
        int         handleWidth  = 13;
        int         handleRadius = 6;
        ...
        qss.append(
            QString(
                "QSlider::handle:horizontal:hover{width:%1px;margin-top:-%2px;"
